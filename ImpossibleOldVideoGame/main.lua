...
function Satellite:approach (dt)
    ...
    if self.miss then
        if self.time > 1 and not self.docked_life then
            self.docked_life = true
            ScoreLives = ScoreLives - 1
            ShakeDuration = Time + 0.5
        end
        return
    end
...
function love.load ()
    ...
    hiscore = Text.new("HISCORE: LEROY 1337", -10, 5, 32, "right")
    lives = Text.new("LIVES: 3", 10, 5, 32, "left")
    title = Text.new("Orbit", 0, TitleY, 112)
    start = Text.new("Press Start", 0, 400, 36)
    lose = Text.new("You Lose", 0, 125, 96)
    again = Text.new("Try Again?", 0, 400, 36)
    ...
    --Radial = moonshine(moonshine.effects.radial)
    --Test = moonshine(moonshine.effects.testbed)
end

function love.draw ()
    if Time < ShakeDuration then
        local dx = love.math.random(-0.5, 0.5)
        local dy = love.math.random(-0.5, 0.5)
...
function love.update (dt)
    Time = Time + dt
    --Test.testbed.time = Time
    --Radial.radial.time = Time
    ...
    elseif SpawnedSatellites < 6 then
        if Time > NextSatellite then
            if SpawnedSatellites == 0 then
                random_satellite(true)
            elseif SpawnedSatellites == 1 then
                OrbitPulseOn = true
                random_satellite(false)
            elseif SpawnedSatellites == 2 then
                random_satellite(true)
            elseif SpawnedSatellites == 3 then
                OrbitPulseOn = true
                random_satellite(false)
            else
                random_satellite(true)
            ...
            SpawnedSatellites = SpawnedSatellites + 1
        end
    elseif EndScreen then
        if Time > EndScreenStartFlash then
            EndScreenStartFlash = EndScreenStartFlash + 0.10
            again.text = "→ Try Again? ←"
            again.color = StartFlashColors[StartFlashIdx]
            if StartFlashIdx == 1 then
                StartFlashIdx = 2
            else
                StartFlashIdx = 1
        ...
        if Time > EndScreenEndFlash then
            lose.display = false
            again.display = false
            Sun.display = false
            Satellites = {}
    ...
    if ScoreLives < 0 then
        lose.display = true
        again.display = true
        lives.display = false
        ScoreLives = 3
        hiscore.text = "HISCORE: LEROY 1337"
        Ended = true
        EndScreen = true
        EndScreenStartFlash = Time + 2.5
        EndScreenEndFlash = Time + 5
    end

    if OrbitPulseOn then
        Pulse.display = true
    ...
    if StartFlashDone and not Ended then
        hiscore.text = "SCORE: " .. tostring(ScoreTally)
        lives.text = "LIVES: " .. tostring(ScoreLives)
    end
