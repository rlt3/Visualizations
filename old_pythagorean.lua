...
function Edge (fromNode, toNode)
    local t = {
        ...
        send_packet = function (self, packet)
            if self.from != packet.pos then
                error("Can't send packet from this start path")
            end
            packet:set_destination(self.toNode)
        ...
        update = function (self, dt)
            ...
            if self.time > 1 then
                ...
                if self.path_index + 1 > #self.path then
                    ...
                    self.pos = self.dest
                    if #AllPackets < 100 then
                        AllPackets:insert(self.dest, math.prandom(1, 3))
                    end
                    return
                end
                self.from = self.to
